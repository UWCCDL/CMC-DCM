...
task_names <- c("Emotion Recognitin", 
...
ggplot(lres, aes(x=Connection, y=Value, col=TaskVsRest)) +
  geom_point(alpha=0.6, position=position_jitter(width=0.2), size=5) +
  stat_summary(data = filter(lres, TaskVsRest=="Task"), fun.data = mean_sdl, geom="errorbar", col="darkred", width=0.25) +
  scale_color_d3() +
  xlab("Connection") +
  ylab("Effective Connectivity (Hz)") +
  ggtitle("Connectivity Values Across Tasks and Rest Data") +
  labs(col="Paradigm") +
  theme_pander() +
